function _wslwrap_register_auto --description 'Register a wrapper function for a command to auto-switch between Windows and WSL filesystems'
    ...
    if functions -q _wslwrap_builtin_$cmd
        function $cmd --inherit-variable cache --inherit-variable cmd --inherit-variable options --description "$marker builtin for $cmd"
            _wslwrap_builtin_$cmd $cache $options $argv
        end
    else
        function $cmd --inherit-variable cache --inherit-variable cmd --inherit-variable options --description "$marker auto for $cmd"
