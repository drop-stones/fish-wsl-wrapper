function _wslwrap_register_auto --description 'Register a wrapper function for a command to auto-switch between Windows and WSL filesystems'
    ...
    if functions -q _wslwrap_builtin_$cmd
        function $cmd --inherit-variable cmd --inherit-variable options --description "$wslwrap_function_marker: builtin for $cmd"
            _wslwrap_builtin_$cmd $options $argv
        end
    else
        function $cmd --inherit-variable cmd --inherit-variable options --description "$wslwrap_function_marker: auto for $cmd"
