function _wslwrap_help --description "Show usage information for wslwrap and related commands"
    ...
    if test (count $argv) -eq 0
        ...
        _wslwrap_echo command -n "  add-path    "
        echo "Add Windows command directories to PATH."
    ...
    switch $subcommand
        ...
        case add-path
            _wslwrap_echo header -n "Usage: "
            _wslwrap_echo command "wslwrap add-path [-a|--append] [-p|--prepend] <command> [<command> ...]"
            echo ""
            echo "Add directories containing specified Windows commands to PATH."
            echo "Commands are resolved using Windows PATH and added to Fish PATH."
            echo ""
            _wslwrap_echo header "Options:"
            _wslwrap_echo command -n "  -a, --append   "
            echo "Add to end of PATH (lower priority, default)."
            _wslwrap_echo command -n "  -p, --prepend  "
            echo "Add to beginning of PATH (higher priority)."
            echo ""
            _wslwrap_echo header "Examples:"
            _wslwrap_echo command "  wslwrap add-path win32yank"
            echo "    Add win32yank's directory to PATH."
            _wslwrap_echo command "  wslwrap add-path --prepend node npm yarn"
            echo "    Add Node.js tool directories to beginning of PATH."
        case help
            _wslwrap_echo header -n "Usage: "
