function _wslwrap_help --description "Show usage information for wslwrap and related commands"
    ...
    if test (count $argv) -eq 0
        ...
        _wslwrap_echo command -n "  which       "
        echo "Resolve a command path, considering wslwrap wrappers."
    ...
    switch $subcommand
        ...
        case which
            _wslwrap_echo header -n "Usage: "
            _wslwrap_echo command "wslwrap which [-a|--all] <command> [<command> ...]"
            echo ""
            echo "Resolve a command path with wslwrap awareness."
            echo "Registered wrappers are resolved to the path the wrapper would invoke"
            echo "at the current directory (Windows .exe inside /mnt/[a-z]/, Linux binary"
            echo "elsewhere). Unregistered commands fall back to the system 'which'."
            echo ""
            _wslwrap_echo header "Options:"
            _wslwrap_echo command -n "  -a, --all  "
            echo "Print all matches (wrapper resolution plus all 'which -a' results)."
            echo ""
            _wslwrap_echo header "Examples:"
            _wslwrap_echo command "  wslwrap which git"
            echo "    Show what 'git' would resolve to."
            _wslwrap_echo command "  wslwrap which -a fd"
            echo "    Show all candidates for 'fd'."
            _wslwrap_echo command "  alias which 'wslwrap which'"
            echo "    Make 'which' wslwrap-aware."
        case help
            _wslwrap_echo header -n "Usage: "
